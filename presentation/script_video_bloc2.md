...
| 5 | 0:40 | Dashboard | Login `admin` / `Bloc2-Demo-2026` (affiché sur la page), onglet Vue d'ensemble puis Performance ML | « L'accès est protégé. On voit les KPIs, les transactions suspectes, puis la dérive et la précision servie mesurées par Evidently. » Si le statut affiche ALERT (dérive > 0,3, réentraînement en délai de carence), le commenter : c'est le monitoring qui fonctionne. |
| 6 | 0:20 | MLflow | Expérience `fraud-detection`, dernier run, puis onglet Models → registre `fraud-detector` (une version par réentraînement) | « Chaque entraînement, manuel ou automatique, est tracé avec ses métriques. » |
...
- Vérifier qu'aucun secret réel n'apparaît à l'écran (contenu de `.env`) ; les identifiants de démo du dashboard peuvent être montrés.
